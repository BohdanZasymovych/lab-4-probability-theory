...
$$
\frac{S_{\mathbf{xx}}}{S_{\mathbf{yy}}} \mid  H_0 = \frac{S_{\mathbf{xx}}\sigma_y^2}{\sigma_x^2S_{\mathbf{yy}}} \sim \mathcal{F}^{n-1}_{m-1}
$$

Where $\mathcal{F}^{n-1}_{m-1}$ is a Fisher distribution with $n-1$ and $m-1$ degrees of freedom.

We can use the fact that:

$$
P(\frac{S_{\mathbf{xx}}}{S_{\mathbf{yy}}} \in \big[\mathcal{F}^{n-1,m-1}_{\alpha/2}, \mathcal{F}^{n-1,m-1}_{1-\alpha/2}\big] \mid  H_0) = 1-\alpha
$$

Equivalently:

$$
P(\frac{S_{\mathbf{xx}}}{S_{\mathbf{yy}}} \notin \big[\mathcal{F}^{n-1,m-1}_{\alpha/2}, \mathcal{F}^{n-1,m-1}_{1-\alpha/2}\big] \mid  H_0) = \alpha
$$

So critical region is:

$$
C(\alpha) = \{(\mathbf{x}, \mathbf{y}) \in \mathbb{R}^n\times\mathbb{R}^m \mid \frac{S_{\mathbf{xx}}}{S_{\mathbf{yy}}} \notin \big[\mathcal{F}^{n-1,m-1}_{\alpha/2}, \mathcal{F}^{n-1,m-1}_{1-\alpha/2}\big]\}
$$

Minimizing the $\alpha$ given sample $(\mathbf{x}, \mathbf{y})$ we will get:

$$
p = 2\min\{F_{\mathcal{F}^{n-1}_{m-1}}(\frac{S_\mathbf{xx}}{S_\mathbf{yy}}), 1-F_{\mathcal{F}^{n-1}_{m-1}}(\frac{S_\mathbf{xx}}{S_\mathbf{yy}})\}
...
$$
C(0.05) = \{(\mathbf{x}, \mathbf{y}) \in \mathbb{R}^n\times\mathbb{R}^m \mid \frac{S_{\mathbf{xx}}}{S_{\mathbf{yy}}} \notin \big[\mathcal{F}^{n-1,m-1}_{0.025}, \mathcal{F}^{n-1,m-1}_{0.975}\big]\}
$$

```{r}
v <- sum((x - mean(x))^2)/sum((y - mean(y))^2)
print(sprintf("Statistic Sxx/Syy: %.6f", v))
...
p <- 2*min(pf(v, length(x)-1, length(y)-1), 1-pf(v, length(x)-1, length(y)-1))
...
So we can see that for the significance level of $0.05$ we reject $H_0$ and conclude that variances do differ. And actually our p-value is extremely small - we will reject the $H_0$ for very small values of $\alpha$. Thus we conclude that we **reject** the null hypothesis and we can say that variances of $Y$ and $X$ are different.
...
## Fitting model and result evaluation
...
$$
t = \frac{\hat{b}-0}{Var(\hat{b})} = \frac{\hat{b}}{\hat{\sigma}^2}S_\mathbf{xx} = \frac{\hat{b}}{\hat{R}}S_\mathbf{xx}(n-2)
$$
